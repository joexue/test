...
#if 0
unsigned short  crc16(char d[], int len)
{
    char b = 0;
...
int main(int argc, char *argv[])
{
    ...
    crc = crc16_table(test, LEN);
    //gettimeofday(&tvafter, NULL);
    //printf("test spend time is %d msecond.\n", (tvafter.tv_sec-tvpre.tv_sec)*1000+(tvafter.tv_usec-tvpre.tv_usec)/1000);
	int s2 = times((struct tms *)NULL);
    printf("crc=%x time=%d \n", crc, s2-s1);
	s1 = times((struct tms *)NULL);
    crc = crc16_ccitt(test, LEN);
	s2 = times((struct tms *)NULL);
    printf("crc=%x time=%d \n", crc, s2-s1);
	s1 = times((struct tms *)NULL);
    crc = crc16(test, LEN);
	s2 = times((struct tms *)NULL);
